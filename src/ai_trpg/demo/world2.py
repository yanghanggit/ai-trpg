...
def create_test_world2() -> World:
    ...
    instance_actor3.initial_context = [
        ...
        AIMessage(
            content=f"我是{instance_actor3.name}。我在 奥顿教堂墓地。我的目标是 进入教堂寻找关于这座城市的真相，尤其是教会档案室中可能隐藏的秘密文献。我需要谨慎行动，观察环境，寻找进入教堂的安全路径。"
        ),
    ]
    #########################
...
**{instance_actor3.name}**: 小心翼翼地站在墓地入口，手持油灯观察四周。他注意到西侧的石阶通往一扇半掩的教堂侧门，那里似乎是唯一的入口。墓地中央的天使雕像在血月下投射出扭曲的阴影，让人不安。他深吸一口气，决定先探索墓地，确认安全后再进入教堂。"""

    # 设置角色状态
    instance_stage1.actor_states = f"""**{instance_actor3.name}**: 墓地南侧入口，距铁栅栏门约5米 | 警惕观察 | 左手持油灯，右手握手杖"""
