...
def create_test_world1() -> World:
    ...
    instance_actor1.initial_context = [
        ...
        AIMessage(
            content=f"我是{instance_actor1.name}。我在 奥顿教堂墓地。我的目标是 杀死任何闯入者！毫不犹豫，直接发起攻击！"
        ),
    ]
    ...
    instance_actor2.initial_context = [
        HumanMessage(content="""# 游戏开始！你是谁？你在哪里？你的目标是什么？"""),
        AIMessage(
            content=f"我是{instance_actor2.name}。我在 奥顿教堂墓地。我的目标是 狩猎 {instance_actor1.name}！因为斯科因已经兽化，所以必须消灭他。我决定要马上出手一击必杀！"
        ),
    ]
    ...
    instance_actor3.initial_context = [
        HumanMessage(content="""# 游戏开始！你是谁？你在哪里？你的目标是什么？"""),
        AIMessage(
            content=f"我是{instance_actor3.name}。我在 奥顿教堂墓地。我的目标是 探索这里的秘密并自保，尽量回避危险，必要时可以反击！"
        ),
    ]
...
**{instance_actor3.name}**: 环顾四周，试图弄清楚自己身处何地以及接下来该做什么。这座被诅咒的城市对他来说充满了未知与危险。"""
