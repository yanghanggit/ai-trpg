...
⚠️ **注意**: 以下4个部分存在逻辑依赖关系，必须按顺序完成

1. **计算战斗/互动结果** ← 基础数据，决定其他内容
...
2. **构建场景叙事** ← 必须与计算结果匹配
   - 基于第1步的计算结果，用第三人称描述行动过程和结果
   - 战斗场景：描述攻击命中、伤害效果、角色反应等
   - 叙事必须与计算数据一致（如伤害量、生死状态等）
   - 注重戏剧性和连贯性

3. **确定角色状态变化** ← 必须与叙事匹配
   - 格式：`**角色名**: 位置 | 姿态 | 状态`
   - 基于第2步的叙事内容，确定每个角色的新位置、姿态和特殊状态
   - 状态变化必须符合叙事描述（如被击中则姿态改变）

4. **更新环境描述** ← 必须与叙事匹配
   - 基于第2步的叙事内容，更新环境变化部分
   - 保持未变化部分，添加战斗/互动导致的环境变化
