...
🚨 **这是唯一需要调用工具的阶段，必须完成所有必需的工具调用**

基于步骤1的分析结果，**按顺序执行**以下工具调用：

#### 1️⃣ 同步场景状态（必须执行）
保存步骤1计算的：计算日志、叙事、角色状态、环境描述

#### 2️⃣ 更新角色生命值（如有变化则必须执行）
- 检查步骤1的计算日志
- 如果有角色生命值变化，**必须**为每个受影响的角色调用工具更新生命值
- 一个角色受伤 = 一次工具调用

#### 3️⃣ 移除已消耗的效果（如有消耗则必须执行）
- 检查步骤1的计算日志
- 如果有效果被触发/消耗，**必须**为每个效果调用工具移除
- 一个效果消耗 = 一次工具调用

⚠️ **注意**: 
- 所有工具调用必须在步骤2完成，不能延迟到步骤3
- 如果计算日志中提到生命值变化或效果消耗，就必须调用相应工具
- 不要遗漏任何需要更新的数据
...
⚠️ **禁止在此阶段调用工具或输出工具调用格式**

输出一段简短的总结（70字以内），确认场景执行已完成。

💡 **提示**: 只需用自然语言简短描述场景结果，不要输出JSON或工具调用格式。"""
